    ...
    <div ref={triggerRef} className={`relative ${className}`}>
      {!open ? (
        <button
          ...
          onClick={openMenu}
          title="Change unit"
          className="flex items-center gap-1 text-[13px] text-muted hover:text-body cursor-pointer"
        >
          <span>{displayFor(value)}</span>
          <svg className="w-3 h-3 text-muted/70" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M2 4l4 4 4-4" />
          </svg>
        </button>
      ) : (
        <input
          ...
          className="w-16 text-[13px] text-body text-right bg-transparent border-b border-border outline-none"
        />
      )}
              ...
              {filtered.map((u) => (
                <li key={u.value}>
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 text-sm cursor-pointer hover:bg-accent/10 ${
                      u.value === value ? "text-accent font-semibold" : "text-body"
                    }`}
                  >
                    {u.label}
                  </button>
              ...
              {query.trim() && !filtered.some((u) => u.value === query.trim()) && (
                <li>
                  <button
                    ...
                    className="w-full text-left px-3 py-1.5 text-sm text-body hover:bg-accent/10 cursor-pointer"
                  >
                    Use “{query.trim()}”
                  </button>
                </li>
