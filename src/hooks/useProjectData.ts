...
/**
 * Fetch a project's server-authoritative state and replace the store with it:
 *   1. Flush anything still queued (so the response includes offline work).
 *   2. Fetch plans, calibrations, measurements, BOQ tree in parallel.
 ...
 *
 * Used by useProjectData on open and by the realtime layer after a socket
 * reconnect (ops broadcast during the outage were missed). Returns
 * 'waiting' when the project's client_uuid is not known yet, 'stale' when
 * `isStale()` reported that another project took over the store mid-fetch,
 * 'done' otherwise.
 */
export const hydrateProjectFromServer = async (
  projectId: string,
  apiClientUuid: string | null,
  isStale: () => boolean = () => false
): Promise<'waiting' | 'stale' | 'done'> => {
  // Skip until the API confirms this project has a real client_uuid.
  const clientUuid = ensureClientUuid(projectId, apiClientUuid);
  if (!clientUuid) {
    console.log(`[project-data] waiting for API client_uuid project=${projectId}`);
    return 'waiting';
  }
  ...
  if (isStale()) {
    console.log(`[project-data] discarding stale hydration for project=${projectId}`);
    return 'stale';
  }
  ...
  if (shouldMigrate) {
    console.log(`[project-data] running one-shot BOQ migration for project=${projectId}`);
    ...
    try {
      ...
      didMigrate = true;
      console.log(`[project-data] BOQ migration complete for project=${projectId}`);
    } catch (error) {
      console.warn(
...
/**
 * Loads a project's server-authoritative state on mount via
 * `hydrateProjectFromServer` (after seeding the store from localStorage so
 * the canvas has something to render before the network answers).
 *
...
export const useProjectData = (
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        await hydrateProjectFromServer(projectId, apiClientUuid, isStale);
      } catch (error) {
